...
REM 1. 檢查檔案與儲存庫大小
REM GitHub 限制：單一檔案嚴格上限為 100MB (104857600 bytes)，整個儲存庫建議保持在 1GB 以下
...
REM 4. 複製真實資料庫並強制加入 Git
REM 為什麼要複製？因為如果直接追蹤 language_learning.db，當您切換回 main 時，Git 會把您的真實資料庫刪除！
REM 用一個分身 (github_backup.db) 來給 Git 追蹤，是最安全的做法。
echo [4/6] 建立資料庫分身並提交...
...
REM 6. 切回原本的分支並還原工作狀態
echo [6/6] 切換回 %CURRENT_BRANCH% 並還原進度...
git checkout %CURRENT_BRANCH% >nul
REM Git 會自動把 github_backup.db 刪除，讓您的目錄保持乾淨 (真實的 language_learning.db 毫髮無傷)
